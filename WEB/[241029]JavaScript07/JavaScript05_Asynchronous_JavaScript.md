# JavaScript05_Asynchronous JavaScript
...
1. XHR 객체 생성 및 요청
2. 서버는 새로운 페이즈를 응답으로 만들지 않고 필요한 부분에 대한 데이터만 처리 후 응답(JSON 및 기타 데이터)
...
부라우저와 Node.js에서 사용할 수 있는 promise 기반의 HTTP 클라이언트 라이브러리
...
- 요청 후 cat api로부터 응답을 기다려야하는 작업은 비동기로 처리하기 때문에 ‘냥냥냥냥’ 출력 이후 응답 데이터가 출력되는 것을 확인할 수 있음
...
⇒ 프론트엔드에서 Axios를 활용해 DRF로 만든 API 서버로 요청을 보낵내고, 받아온 데이터를 비동기적으로 처리하는 로직을 작성하게 됨
...
- 비동기 코드를 작성하다보면 콜백 함수로 인한 콜백 지옥은 빈번히 나타나는 문제이며 이는 코드의 가독성을 해치고 유지 보수가 어려워짐
...
4. 에러 처리의 일원화
    ...
    - 전통적인 콜백 방식에서 각 콜백마다 에러 처리를 해야 하는 번거로움 해소
